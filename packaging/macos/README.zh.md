...
3. Tauri 壳启动 `dsh web --host 127.0.0.1 --port 0`（由操作系统分配空闲端口），等待 `dsh web: http://…` 就绪行后在应用窗口加载该 URL；退出应用或关闭主窗口时停止服务。窗口使用 overlay 标题栏（原生红绿灯叠在 web UI 上，无标题栏分隔线；[说明](../../.agents/notes/implemented/feature/2026-08-15-macos-immersive-title-bar.md)）。对话中的 `http(s)://…` 链接（含其他 localhost 端口）在系统浏览器中打开；嵌入式 web UI origin 留在 WebView（[说明](../../.agents/notes/implemented/bug-fix/2026-08-14-macos-desktop-http-links-system-browser.md)）。
