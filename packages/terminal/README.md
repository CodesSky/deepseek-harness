...
The design and deferred boundaries live in the [persistent PTY Agent Note](../../.agents/notes/implemented/feature/2026-07-16-persistent-pty-sessions.md). Shipped `standard` / `code` / `cordis` agent presets mount this family on Linux and macOS; `minimal` uses the single-tool persistent bash consumer instead.
