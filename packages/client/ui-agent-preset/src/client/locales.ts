...
export const en: Record<AgentPresetSettingsKey, string> = {
  ...
  presetStandardDescription:
    'Full coding agent with file editing, one-shot shell, persistent terminal (PTY), file and web search, skills, planning, goals, subagents, and workflows.',
  presetCodeName: 'Code mode',
  presetCodeDescription:
    'All Standard mode capabilities (including persistent PTY), with tools exposed through the Code Mode SDK so the model can combine multi-step operations in one TypeScript program.',
  presetMinimalName: 'Minimal mode',
  presetMinimalDescription:
    'Two-tool coding agent with persistent bash and str_replace_editor.',
  presetCordisName: 'Creator mode',
  presetCordisDescription:
    'Built for creating custom agent presets, with all Standard mode capabilities (including persistent PTY) plus runtime inspection, plugin experiments, and preset-authoring guidance.',
  duplicate: 'Duplicate',
...
export const zh: Record<AgentPresetSettingsKey, string> = {
  ...
  presetStandardDescription: '功能完整的编码 Agent，支持文件编辑、一次性 Shell、持久终端（PTY）、文件与网页检索、Skills、计划、目标、子代理和工作流。',
  presetCodeName: 'PTC 模式',
  presetCodeDescription: '具备标准模式的全部能力（含持久终端 PTY），并通过 Code Mode SDK 呈现工具，让模型用一个 TypeScript 程序组合多步操作。',
  presetMinimalName: '极简模式',
  presetMinimalDescription: '仅提供持久 bash 与 str_replace_editor 的双工具编码 Agent。',
  presetCordisName: '创造模式',
  presetCordisDescription: '用于创建自定义 Agent preset：具备标准模式的全部能力（含持久终端 PTY），并提供运行时检查、插件实验和 preset 创作指导。',
  duplicate: '复制',
